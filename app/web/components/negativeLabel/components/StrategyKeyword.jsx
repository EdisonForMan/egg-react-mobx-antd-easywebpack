...
@inject(stores => ({
  store: stores.strategyKeywordStore
}))
@hoc({ name: '', className: 'page_strategyKeyword' })
@observer
class StrategyKeyword extends Component {
  state = {
    loading: false,
    savingLoad: false,
  ...
  columns() {
    const { statusOption, cautionLevelOption, dataTagOption } = this.state;
...
export default StrategyKeyword;
