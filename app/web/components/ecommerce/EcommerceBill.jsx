...
import BillForm, {
  BILL_FORM_HASH,
  BILL_FORM_MODE_ADD,
  BILL_FORM_MODE_UPDATE
} from './components/BillForm';
import hoc from 'components/HOC/pageHeader';
const dateFormat = 'YYYY-MM-DD';

@inject(stores => ({
  store: stores.ecommerceBillStore
}))
@hoc({ name: '已入库数据-账单', className: 'page_ecommerceBill' })
@observer
class EcommerceBill extends Component {
  state = {
    loading: false,
    savingLoad: false,
    pendingFormModalVisiable: false,
    pendingFormMode: BILL_FORM_MODE_ADD,
    edit: null,
    treeData: [],
    treeOption: [],
    statusOption: [
    ...
    return (
      <span className="action-left-search">
        ...
        <span className="action-left-search-single">
          <label>入库数据状态:</label>
          <Select
            defaultValue={_query.isEnabled}
            style={{ width: '100px' }}
            onChange={val => {
              _query.isEnabled = val;
            }}
          >
        ...
        <Button
          type="primary"
          icon="plus"
          onClick={() => this.openModal(BILL_FORM_HASH)}
        >
          新增
        </Button>
      </span>
  ...
  columns() {
    const { expressionOption, statusOption } = this.state;
    return [
      {
        title: '序号',
        width: 80,
      ...
      },
      {
        ...
        render: (t, r) => {
          return (
            <a
              href="javascript:"
              onClick={() => this.openModal(BILL_FORM_HASH, r)}
            >
              {t}
            </a>
          );
  ...
  async onSave() {
    ...
    form.validateFieldsAndScroll(async (err, values) => {
      ...
      if (values.rearExpressionText) {
        ...
        values.expression = values.rearExpressionText;
      }
      this.setState({ savingLoad: true });
      try {
        const data = await editEcommerceData(values);
        this.hideModal(BILL_FORM_HASH);
    ...
    switch (hash) {
      case BILL_FORM_HASH: {
        this.setState({
          ...
          pendingFormMode: obj ? BILL_FORM_MODE_UPDATE : BILL_FORM_MODE_ADD,
          edit: { ...obj, keyword: obj ? obj.keyword : undefined }
        });
        break;
      }
    }
    ...
    return (
      <div>
        ...
        <Modal
          ...
          onCancel={() => this.hideModal(BILL_FORM_HASH)}
          footer={[
            <Button key="back" onClick={() => this.hideModal(BILL_FORM_HASH)}>
              取消
            </Button>,
            <Button
              key="submit"
